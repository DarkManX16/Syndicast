...
module.exports = function ($timeout, dizquetv, getShowData, seasonConstraints) {
    ...
    return {
        ...
        link: function (scope, element, attrs) {
            ...
            /*
             * Season settings belong to the slot, so two slots naming the same show
             * can run different ranges of it. Slots that ask for the same seasons
             * still share one episode position, so they advance through the range
             * together rather than each starting over.
             *
             * The open panel is held by slot reference rather than by index, to
             * match the time slots editor, where the array is sorted in place.
             */
            scope.openSeasonEditor = null;
