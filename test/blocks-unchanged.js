/*
 * A channel with no day-parts must behave exactly as it did before day-parts
 * existed - the spec's own acceptance rule. When this was first verified (see
 * NOTES.md), that meant diffing this code against the previous commit's
 * helperFuncs.js/channel-cache.js extracted with `git show`. That one-time
 * comparison isn't kept here: pinned to a commit, it would either rot as
 * history moves past it or break outright if that commit is ever rewritten,
 * for a guarantee that only needs proving once per change to this code path,
 * not replayed on every future `npm test`.
 *
 * What's kept is the same three guarantees, restated as self-contained
 * assertions instead of a diff:
 ...
 *   - the picker is handed the very same array object a channel without
 *     day-parts was always handed, not a copy or a rebuild
 *   - a realistic run's distribution is still governed by the configured
 *     list weights, not driven off course by day-parts machinery
 */
...
module.exports = async function run() {
    ...
    // 2. Identity: no day-parts means the resolver hands back the channel's
    // own array, unchanged, and declines to compute a mix at all.
    {
        const ch = plainChannel(mix([['L', 100]]));
        ...
        suite.check('resolveCollections declines to intervene when there are no day-parts',
