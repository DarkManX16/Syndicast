...
function video( channelService, fillerDB, db, programmingService, activeChannelService, programPlayTimeDB ) {
    ...
    router.get('/setup', (req, res) => {
        ...
        console.log(`\r\nStream starting. Channel: 1 (Syndicast)`)

        let ffmpeg = new FFMPEG_TEXT(ffmpegSettings, 'Syndicast (No Channels Configured)', 'Configure your channels using the Syndicast Web UI')
        ...
        res.on('close', () => { // on HTTP close, kill ffmpeg
            ffmpeg.kill()
            console.log(`\r\nStream ended. Channel: 1 (Syndicast)`)
        })
    })
